...
schSpe0_75 <- map(schSpe0_75, function(x) {
    x$probes <- gsub("_mouse|_rat", "", x$probes)
    return(x)
})
...
schSpeCluster0_75 <- map(unique(schypeOutput0_75$cluster_id), function(x) {
    myCluster <- filter(schypeOutput0_75, cluster_id == x)
    mir <- list(
        mouse = filter(myCluster, grepl("_mouse", probes, fixed = TRUE)) %>%
            mutate(probes = gsub("_mouse", "", probes)),
        rat = filter(myCluster, grepl("_rat", probes, fixed = TRUE)) %>%
...
clusterSize0_75 <- map_int(schSpeCluster0_75, function(x) {
    min(length(x$mouse), length(x$rat))
})
schSpeCluster0_75 <- schSpeCluster0_75[clusterSize0_75 >= 2]
